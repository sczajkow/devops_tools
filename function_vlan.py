...
qrack = list(range(2,4)) #single host test variable
rack_switches = list(range(1,2)) #single host test variable
row = ('a','r')
...
def get_vlan():
    for rack in qrack:
        for index in rack_switches:
            host = f'ful01-130-q{rack}-tor-{index}.devit.ciscolabs.com'
...
def find_single_vlan():
    ...
    for v,h in switch_vlan.items():
        if v == input_vlan:
            host = h
    if vlan is None:
        print('This Vlan is Not Assigned')
    else:
